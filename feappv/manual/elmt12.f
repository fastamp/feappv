...
c-----[--+---------+---------+---------+---------+---------+---------+-]
      ...
      logical    errck, pcomp, pinput, tinput, start

      integer    ndf,ndm,nst,isw
      integer    i,j, i1,j1, l,lint, nhv,nn
      real*8     xx,yy, xsj, a1,a2,bd1,bd2, shj, torque

      integer    ix(*)
      real*8     d(*),ul(ndf,*),xl(ndm,*),tl(*),s(nst,*),p(*)
...
c         Compute strains
          ...
          call stra12(xl,ul, xx,yy,shp,gam, ndm,ndf)

c         Compute stresses

          call stre12(d,gam,hr(nh1+nn),hr(nh2+nn),tau,dd, region)
...
c     Compute nodal tau for print/plots
      ...
      elseif(isw.eq.8) then
        ...
        call stcn12(ix,d,xl,ul,s,shp,hr(nph),hr(nph+numnp),hr(ner),erav,
     &              ndf,ndm,nel,numnp)
...
c     Compute error data for tau
      ...
      elseif(isw.eq.9 .or. isw.eq.11) then
        ...
        call ster12(ix,d,xl,ul,shp,hr(nph+numnp),ndf,ndm,nel,numnp)
...
2000  format(5x,'Elastic St. Venant Torsion Element'//
     &  5x,'Shear Modulus ',e12.5/)

2001  format(5x,'Elastic-Plastic (Mises) St. Venant Torsion Element'//
     &  5x,'Shear Modulus ',e12.5/5x,'Shear Yield   ',e12.5/
     &  5x,'Isotropic H   ',e12.5/)

2002  format( 5x,'Error Factor',e13.5/)

2003  format(a1,20a4//5x,'Element Stress'//' Elmt Matl 1-coord  2-coord'

     &      ,'    1-stress    2-stress    1-strain    2-strain'/
     &   5x,'State    Ratio    Yield  e_p-strain      Torque')

2004  format(2i5,2f9.3,4e12.3/3x,a7,1p,2e9.2,1p,2e12.3)

3000  format(' Input: type (elas,mises), mu, tau_0, H_iso '/'  1>',$)

3001  format(' Input: error factor'/'   2>',$)

      end

      subroutine stcn12(ix,d,xl,ul,s,shp,dt,st,ser,erav,
     &                  ndf,ndm,nel,numnp)
...
c       Compute lumped projection and assemble stress integrals
      ...
      subroutine stra12(xl,ul, xx,yy,shp,gam, ndm,ndf)
...
c     Compute gamma
      ...
      subroutine ster12(ix,d,xl,ul,shp,st,ndf,ndm,nel,numnp)
      ...
      real*8     d(*),gam(2),tau(2),ul(ndf,*),sg(3,9)
...
c     Check for triangle
     ...
     &            .or. ix(3).eq.ix(4) .or. ix(4).eq.ix(1) ) then
      ...
      subroutine stre12(d,gam,epn,ep1,tau,dd, region)
...
c-----[-+---------+---------+---------+---------+---------+---------+-]
      ...
      real*8     d(*), gam(4), epn(*), ep1(*), tau(5), dd(2,2)
...
c       Check for plasticity and apply corrector if necessary
        ...
        call plas12(d(1),epn,ep1, tau, dd, region)
...
c       Print elastic plastic tangent
      ...
      subroutine plas12(d,epn,ep1, sig, dd, region)
